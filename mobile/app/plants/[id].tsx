...
const getPlantIcon = (type: string) => {
    switch (type.toLowerCase()) {
        ...
        case 'flower':
            return <Flower size={24} color="#F59E0B" />;
        case 'fruit':
            return <Apple size={24} color="#10B981" />;
        default:
            return <Leaf size={24} color="#10B981" />;
    }
};

const getStatusIcon = (status: PlantScan['status']) => {
    switch (status) {
        case 'healthy':
...
export default function PlantDetailScreen() {
    ...
    // const plantScans = mockPlantScans.filter(scan =>
    //     scan.plantName.toLowerCase() === plant?.name.toLowerCase()
    // );
    if (loading) {
        return (
                ...
                <View className="relative h-72">
                    ...
                    <View className="absolute bottom-0 left-0 right-0 bg-black/60 p-5 flex-row items-center">
                        ...
                        <View className="flex-1">
                            ...
                            <View style={[styles.healthBadge, { backgroundColor: PlantUtils.getHealthColor(plant.health) }]}> {/* à convertir plus tard */}
                                <Text className="text-xs font-semibold text-white">
                                    {plant.health === 'healthy' ? 'Healthy' :
                                        plant.health === 'warning' ? 'Needs Attention' : 'Critical'}
                                </Text>
                            </View>
                        </View>
                    </View>
                    ...
                    <View className="space-y-3">
                        ...
                        <View className="bg-white p-5 rounded-2xl shadow-sm space-y-4">
                            <View className="flex-row justify-between items-center">
                                ...
                                <Text className="text-base text-slate-900 font-semibold text-right flex-1">{plant.type}</Text>
                            </View>
                            {plant.variety && (
                                <View className="flex-row justify-between items-center">
                                    <Text className="text-base text-slate-500 font-medium">Variety:</Text>
                                    <Text className="text-base text-slate-900 font-semibold text-right flex-1">{plant.variety}</Text>
                                </View>
                            )}
                            {plant.lastScanned && (
                                <View className="flex-row justify-between items-center">
                                    <Text className="text-base text-slate-500 font-medium">Planted:</Text>
                                    <Text className="text-base text-slate-900 font-semibold text-right flex-1">{PlantUtils.formatFullDate(plant.createdAt)}</Text>
                                </View>
                            )}
                            {plant.location?.address && (
                                <View className="flex-row justify-between items-center">
                                    <Text className="text-base text-slate-500 font-medium">Location:</Text>
                                    <Text className="text-base text-slate-900 font-semibold text-right flex-1">{plant.location.address}</Text>
                                </View>
                            )}
                        </View>
                    </View>
                    ...
                    <View className="space-y-3">
                        ...
                        {plantScans.length > 0 ? (
                            <View className="space-y-4">
                                {plantScans.map((scan) => (
                                    <View key={scan.id} className="bg-white rounded-2xl p-5 shadow-sm">
                                        <View className="flex-row justify-between items-center mb-3">
                                            ...
                                            <Text className="text-sm text-slate-500">{PlantUtils.formatDate(scan.createdAt)}</Text>
                                        </View>
